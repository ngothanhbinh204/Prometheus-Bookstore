...
if (isset($_GET['act'])) {
    ...
    switch ($act) {
        ...
        case 'addProduct':
            ...
            if (isset($_POST['button_addProduct'])) {
                ...
                if (isset($_FILES['ufile'])) {
                    ...
                    $lastInsertID = insert_product($title, $isbn_13, $price, $book_description, $fm_id, $au_id, $pub_id, $creation_date);
        ...
        case 'listBlog':
            $listBlog = get_blog_all();
            include "blog/listBlog.php";
            break;
            //  thêm
        case 'addBlog':
            if (isset($_POST['button_addBlog'])) {
                $title = $_POST['title'];
                $admin_id = $_POST['iduseradmin'];
                $content = $_POST['content'];
                $ngayxuatbang = $_POST['creation_date'];
                $ten_file_hinh = $_FILES['img']['name'];
                if ($ten_file_hinh != "") {
                    // upload lên host
                    $target_file = "../publics/img/blog/" . $ten_file_hinh;
                    move_uploaded_file($_FILES["img"]["tmp_name"], $target_file);
                }
                //thêm vào database
                add_blog($admin_id, $title, $content, $ngayxuatbang, $ten_file_hinh);
                $thongbao = "Thêm Tin Tức Mới Thành Công !!!";
            }
            $list_userAdmin = list_userAdmin();
        ...
        case 'updateBlog1':
            if (isset($_POST['button_updateBlog'])) {
                $id = $_POST['idblog'];
                $title = $_POST['title'];
                $admin_id = $_POST['iduseradmin'];
                $content = $_POST['content'];
                ...
                if ($ten_file_hinh != "") {
                    ...
                    $hinhcu = "../publics/img/blog/" . $_POST['hinhcu'];
                    if (file_exists($hinhcu)) unlink($hinhcu);
                }
                update_blog($admin_id, $title, $content, $ngayxuatbang, $ten_file_hinh, $id);
                $thongbao = "Cập Nhật Tin Tức Mới Thành Công !!!";
            }
            $get_oneBlog = get_oneBlog($id);
            $list_userAdmin = list_userAdmin();
            include "Blog/updateBlog.php";
        ...
        case 'delBlog':
            if (isset($_GET['id']) && ($_GET['id'] > 0)) {
                $id = $_GET['id'];
                // kiểm tra sản phẩm có hình  không? có thì xóa
                $ten_file_hinh = "../publics/img/blog/" . get_ten_filehinh($id);
            ...
            if (isset($_POST['updateTrangthai'])) {
                ...
                if (isset($_POST['update'])) {
                    foreach ($_POST['update'] as $updateid) {
                        $trangthai = $_POST['trangthai_' . $updateid];
                        if ( $trangthai != '') {
                            update_trangthai($updateid, $trangthai);
                        }
                    }
                }
            }
            $listOrders = load_all_fetch_order_details_admin();
