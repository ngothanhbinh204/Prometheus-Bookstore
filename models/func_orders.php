...
{
    $sql = "UPDATE order_details SET  trangthai = ? WHERE id = ?";
...
function load_all_fetch_order_details_admin()
{
    $sql = "SELECT order_details.id as id , order_details.delivery_note as delivery_note, order_details.total_order as total_order, order_details.order_code as order_code, order_details.creation_date as creation_date , payment_methods.pm_name as pm_name, order_details.trangthai as trangthai
    FROM `order_details` 
    INNER JOIN payment_methods on payment_methods.id =  order_details.pm_id
    WHERE 1";
